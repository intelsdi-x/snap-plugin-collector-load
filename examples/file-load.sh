...
(cd $PLUGIN_PATH && curl -sSO http://snap.ci.snap-telemetry.io/plugins/snap-plugin-publisher-file/latest_build/linux/x86_64/snap-plugin-publisher-file && chmod 755 snap-plugin-publisher-file)
            ...
            _info "try ${i}"
             if [[ -f /usr/local/bin/snaptel && -f /usr/local/sbin/snapteld ]];
                then
                    ...
                    snaptel task create -t "${__dir}/tasks/task-load.json"
